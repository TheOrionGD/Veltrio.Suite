...
async function ensureDbConnected() {
  if (!isDbConnected) {
    const mongoUrl = process.env.VITE_MONGODB_URL || process.env.MONGODB_URI;
    const dbName = process.env.VITE_MONGODB_DB_NAME || 'VELTRIO';
    if (!mongoUrl) {
      throw new Error('Database connection URL (VITE_MONGODB_URL or MONGODB_URI) is missing.');
    }
    await connectToDatabase(mongoUrl, dbName);
    isDbConnected = true;
  }
}

export default async function handler(req: any, res: any) {
  // CORS Headers for Vercel API environment
